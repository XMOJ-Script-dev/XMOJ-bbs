      ...
      for (const i in OtherUsernameList) {
        ...
        if (LastMessageFrom.toString() === "") {
          ...
          if (LastMessage[0]["content"].startsWith("Begin xssmseetee v1 encrypted message")) {
            LastMessage[0]["content"] = CryptoJS.AES.decrypt(LastMessage[0]["content"].substring(38), this.shortMessageEncryptKey + Data["OtherUser"] + this.Username).toString(CryptoJS.enc.Utf8);
          } else {
          ...
          if (LastMessage[0]["content"].startsWith("Begin xssmseetee v1 encrypted message")) {
            LastMessage[0]["content"] = CryptoJS.AES.decrypt(LastMessage[0]["content"].substring(38), this.shortMessageEncryptKey + this.Username + Data["OtherUser"]).toString(CryptoJS.enc.Utf8);
          } else {
            let preContent = LastMessage[0]["content"];
            LastMessage[0]["content"] = "无法解密消息, 原始数据: " + preContent;
          }
    ...
    GetMail: async (Data: object): Promise<Result> => {
      ...
      for (const i in Mails) {
        ...
        if (Mail["content"].startsWith("Begin xssmseetee v1 encrypted message")) {
          Mail["content"] = CryptoJS.AES.decrypt(Mail["content"].substring(38), this.shortMessageEncryptKey + Mail["message_from"] + Mail["message_to"]).toString();
        } else {
